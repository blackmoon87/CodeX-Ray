...
```bash
cat > $PROJECT_ROOT/.codex-ray/tmp/ua-arch-input.json << 'ENDJSON'
{
...
```bash
node $PROJECT_ROOT/.codex-ray/tmp/ua-arch-analyze.js $PROJECT_ROOT/.codex-ray/tmp/ua-arch-input.json $PROJECT_ROOT/.codex-ray/tmp/ua-arch-results.json
```
...
After the script completes, read `$PROJECT_ROOT/.codex-ray/tmp/ua-arch-results.json`. Use the structural analysis as the primary input for your layer decisions. Do NOT re-read source files or re-analyze imports -- trust the script's results entirely.
...
1. Write the JSON array to: `<project-root>/.codex-ray/intermediate/layers.json`
