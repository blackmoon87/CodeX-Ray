...
Execute the pre-built structural extraction script bundled with the CodeX-Ray plugin. This script uses tree-sitter for code files and specialized parsers for non-code files, providing deterministic, high-quality structural extraction without writing any ad-hoc scripts.
...
```bash
cat > $PROJECT_ROOT/.codex-ray/tmp/ua-file-analyzer-input-<batchIndex>.json << 'ENDJSON'
{
...
node <SKILL_DIR>/extract-structure.mjs \
  $PROJECT_ROOT/.codex-ray/tmp/ua-file-analyzer-input-<batchIndex>.json \
  $PROJECT_ROOT/.codex-ray/tmp/ua-file-extract-results-<batchIndex>.json
```
...
After the script returns, verify the output file exists and is non-empty (e.g. `test -s $PROJECT_ROOT/.codex-ray/tmp/ua-file-extract-results-<batchIndex>.json`). Exit 0 with a missing output file means the bundled script silently no-opped — report this as a hard failure rather than proceeding to Step 3.

### Step 3 — Read the extraction results

Read `$PROJECT_ROOT/.codex-ray/tmp/ua-file-extract-results-<batchIndex>.json`. The output format is:
...
**Step B — Decide split.**
- If `nodeCount ≤ 60` AND `edgeCount ≤ 120`: write ONE file to `.codex-ray/intermediate/batch-<batchIndex>.json`. Done. Skip to Step F.
...
Write part `k` (1-indexed) to `.codex-ray/intermediate/batch-<batchIndex>-part-<k>.json`. Each part is a valid GraphFragment: `{ "nodes": [...], "edges": [...] }`.
