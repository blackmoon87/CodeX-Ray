...
2. Check that `$PROJECT_ROOT/.codex-ray/knowledge-graph.json` exists.
   - If not: report "No existing knowledge graph found. Run `/understand` first to create one." and **STOP**.

3. Check that `$PROJECT_ROOT/.codex-ray/meta.json` exists and read `gitCommitHash`.
...
9. **Apply `.understandignore` exclusions** (same semantics as `/understand` Step 2.5 in `agents/project-scanner.md`).
   ...
   1. If neither `$PROJECT_ROOT/.codex-ray/.understandignore` nor `$PROJECT_ROOT/.understandignore` exists, the step 7 extension filter is sufficient — skip to Phase 1.

   2. Write the step 7 file list to `$PROJECT_ROOT/.codex-ray/intermediate/changed-files-pre.json` as a JSON array of relative paths.

   3. Resolve `$PLUGIN_ROOT`:
      - Use `$CLAUDE_PLUGIN_ROOT` if set (Claude Code's hook context sets this).
      - Otherwise try `$HOME/.codex-ray-plugin`.
      - Validate the chosen candidate by checking `$candidate/packages/core/dist/ignore-filter.js` exists.
      - If neither resolves: report "Cannot locate plugin install at `$CLAUDE_PLUGIN_ROOT` or `$HOME/.codex-ray-plugin`; auto-update aborted. Run `/understand` to re-baseline." and **STOP**. Do **not** silently skip — silent skip reproduces issue #153.

   4. Write `$PROJECT_ROOT/.codex-ray/intermediate/ignore-filter.mjs`:
      ```javascript
      ...
      writeFileSync(
        path.join(PROJECT_ROOT, '.codex-ray/intermediate/changed-files.json'),
   ...
   5. Run it:
      ```bash
      node $PROJECT_ROOT/.codex-ray/intermediate/ignore-filter.mjs \
        "$PLUGIN_ROOT" \
        $PROJECT_ROOT/.codex-ray/intermediate/changed-files-pre.json
      ```

   6. Read `$PROJECT_ROOT/.codex-ray/intermediate/changed-files.json`. Pass the `kept` array as the input file list for Phase 1's fingerprint-check script.
...
1. Write and execute a Node.js script (`$PROJECT_ROOT/.codex-ray/intermediate/fingerprint-check.mjs`):

```javascript
// The script should:
// 1. Read fingerprints.json from .codex-ray/fingerprints.json
...
// 6. Write result to .codex-ray/intermediate/change-analysis.json
```
...
2. Read `.codex-ray/intermediate/change-analysis.json`.
...
1. Read the existing knowledge graph from `$PROJECT_ROOT/.codex-ray/knowledge-graph.json`.
...
3. For each batch, dispatch a subagent using the `file-analyzer` agent definition (at `agents/file-analyzer.md`). Append:
   ...
   > Languages: `<languages>`
   > Batch index: `1`
   > Write output to: `$PROJECT_ROOT/.codex-ray/intermediate/batch-1.json`
   >
...
2. Write updated metadata to `$PROJECT_ROOT/.codex-ray/meta.json`:
   ```json
   {
...
3. **Update fingerprints (LOAD-PATCH-SAVE, not OVERWRITE).**
   ...
   const fpPath = path.join(PROJECT_ROOT, '.codex-ray', 'fingerprints.json');
...
5. Report a summary:
   ...
   - Path to output: `$PROJECT_ROOT/.codex-ray/knowledge-graph.json`
