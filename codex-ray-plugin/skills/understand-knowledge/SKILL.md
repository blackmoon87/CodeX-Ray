...
2. Run the format detection script bundled with this skill:
   ...
   - If successful, proceed. The script writes `scan-manifest.json` to `<TARGET_DIR>/.codex-ray/intermediate/`
...
3. For each batch, dispatch an `article-analyzer` subagent with:
   ...
   - The intermediate directory path: `$INTERMEDIATE_DIR = <TARGET_DIR>/.codex-ray/intermediate`
...
3. Copy the validated graph to `<TARGET_DIR>/.codex-ray/knowledge-graph.json`

4. Write metadata to `<TARGET_DIR>/.codex-ray/meta.json`:
   ```json
   {
...
5. Clean up intermediate files:
   ```
   rm -rf <TARGET_DIR>/.codex-ray/intermediate
   ```
