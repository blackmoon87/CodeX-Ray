...
Analyze the current code changes against the knowledge graph at `.codex-ray/knowledge-graph.json`.
...
1. Check that `.codex-ray/knowledge-graph.json` exists. If not, tell the user to run `/understand` first.
...
8. **Write diff overlay for dashboard** — after producing the analysis, write the diff data to `.codex-ray/diff-overlay.json` so the dashboard can visualize changed and affected components. The file contains:
   ```json
   {
     "version": "1.0.0",
   ...
   ```
   After writing, tell the user they can run `/CodeX-Ray:understand-dashboard` to see the diff overlay visually.
