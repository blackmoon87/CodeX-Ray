...
Analyze the current codebase and produce a `knowledge-graph.json` file in `.codex-ray/`. This file powers the interactive dashboard for exploring the project's architecture.

## Options

- `$ARGUMENTS` may contain:
  - `--full` — Force a full rebuild, ignoring any existing graph
  - `--auto-update` — Enable automatic graph updates on commit (writes `autoUpdate: true` to `.codex-ray/config.json`)
  - `--no-auto-update` — Disable automatic graph updates (writes `autoUpdate: false` to `.codex-ray/config.json`)
  - `--review` — Run full LLM graph-reviewer instead of inline deterministic validation
  - `--language <lang>` — Generate all textual content (summaries, descriptions, tags, titles, languageNotes, languageLesson) in the specified language. Accepts ISO 639-1 codes (`zh`, `ja`, `ko`, `en`, `es`, `fr`, `de`, etc.) or friendly names (`chinese`, `japanese`, `korean`, `english`, `spanish`, etc.). Locale variants supported: `zh-TW`, `zh-HK`, etc. Defaults to `en` (English). Stores preference in `.codex-ray/config.json` for consistency across incremental updates.
...
1. **Resolve `PROJECT_ROOT`:**
   ...
   - **Worktree redirect.** If `PROJECT_ROOT` is inside a git worktree (not the main checkout), redirect output to the main repository root. Worktrees managed by Claude Code are ephemeral — `.codex-ray/` written there is destroyed when the session ends, taking the knowledge graph with it (issue #133). Detect a worktree by comparing `git rev-parse --git-dir` against `git rev-parse --git-common-dir`; in a normal checkout or submodule they resolve to the same path, in a worktree they differ and the parent of `--git-common-dir` is the main repo root.
...
1.5. **Ensure the plugin is built.** Later phases invoke Node scripts that import `@codex-ray/core`. On a fresh install `packages/core/dist/` does not exist yet — build once.
   ...
   **Important:** do **not** assume the plugin root is simply two directories above the skill path string. In many installations `~/.agents/skills/understand` is a symlink into the real plugin checkout. Prefer runtime-provided plugin roots first (for Claude), then fall back to universal symlinks, skill symlink resolution, and common clone-based install paths.
   ...
   for candidate in \
     "${CLAUDE_PLUGIN_ROOT}" \
     "$HOME/.codex-ray-plugin" \
     "$SELF_RELATIVE" \
     "$COPILOT_SELF_RELATIVE" \
     "$HOME/.codex/CodeX-Ray/CodeX-Ray-plugin" \
     "$HOME/.opencode/CodeX-Ray/CodeX-Ray-plugin" \
     "$HOME/.pi/CodeX-Ray/CodeX-Ray-plugin" \
     "$HOME/CodeX-Ray/CodeX-Ray-plugin"; do
   ...
   if [ -z "$PLUGIN_ROOT" ]; then
     echo "Error: Cannot find the CodeX-Ray plugin root."
     echo "Checked:"
     echo "  - ${CLAUDE_PLUGIN_ROOT:-<unset CLAUDE_PLUGIN_ROOT>}"
     echo "  - $HOME/.codex-ray-plugin"
     ...
     echo "  - $HOME/.codex/CodeX-Ray/CodeX-Ray-plugin"
     echo "  - $HOME/.opencode/CodeX-Ray/CodeX-Ray-plugin"
     echo "  - $HOME/.pi/CodeX-Ray/CodeX-Ray-plugin"
     echo "  - $HOME/CodeX-Ray/CodeX-Ray-plugin"
     echo "Make sure the plugin is installed correctly."
     exit 1
   fi

   if [ ! -f "$PLUGIN_ROOT/packages/core/dist/index.js" ]; then
     cd "$PLUGIN_ROOT" && (pnpm install --frozen-lockfile 2>/dev/null || pnpm install) && pnpm --filter @codex-ray/core build
   fi
   ```
...
3.5. **Auto-update configuration:**
    - If `--auto-update` is in `$ARGUMENTS`: write `{"autoUpdate": true}` to `$PROJECT_ROOT/.codex-ray/config.json`
    - If `--no-auto-update` is in `$ARGUMENTS`: write `{"autoUpdate": false}` to `$PROJECT_ROOT/.codex-ray/config.json`
 ...
 3.6. **Language configuration:**
    ...
    - If `--language` is NOT specified:
      - Check `$PROJECT_ROOT/.codex-ray/config.json` for an existing `outputLanguage` field. If present, use that.
      - If no stored preference, default to `en` (English).
    - If `--language` IS specified:
      - Update `$PROJECT_ROOT/.codex-ray/config.json` with the new language: merge `{"outputLanguage": "<lang>"}` into existing config.
 ...
 4. **Check for subdomain knowledge graphs to merge:**
   List all `*knowledge-graph*.json` files in `$PROJECT_ROOT/.codex-ray/` **excluding** `knowledge-graph.json` itself (e.g. `frontend-knowledge-graph.json`, `backend-knowledge-graph.json`). If any subdomain graphs exist, run the merge script bundled with this skill (located next to this SKILL.md file — use the skill directory path, not the project root):
   ```bash
...
5. Check if `$PROJECT_ROOT/.codex-ray/knowledge-graph.json` exists. If it does, read it.
6. Check if `$PROJECT_ROOT/.codex-ray/meta.json` exists. If it does, read it to get `gitCommitHash`.
7. **Decision logic:**
   ...
   **Review-only path:** Copy the existing `knowledge-graph.json` to `$PROJECT_ROOT/.codex-ray/intermediate/assembled-graph.json`, then jump directly to Phase 6 step 3.
...
1. Check if `$PROJECT_ROOT/.codex-ray/.understandignore` exists.
2. **If it does NOT exist**, generate a starter file:
   - Run the following Node.js one-liner in `$PROJECT_ROOT` (reads `.gitignore` and deduplicates against built-in defaults):
     ...
     const outDir = path.join(root, '.codex-ray');
     if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
     fs.writeFileSync(path.join(outDir, '.understandignore'), header + body);
     "
     ```
   - Report to the user:
     > Generated `.codex-ray/.understandignore` with suggested exclusions based on your project structure. Please review it and uncomment any patterns you'd like to exclude from analysis. When ready, confirm to continue.
   - **Wait for user confirmation before proceeding.**
3. **If it already exists**, report:
   > Found `.codex-ray/.understandignore`. Review it if needed, then confirm to continue.
   - **Wait for user confirmation before proceeding.**
...
> Write output to: `$PROJECT_ROOT/.codex-ray/intermediate/scan-result.json`

After the subagent completes, read `$PROJECT_ROOT/.codex-ray/intermediate/scan-result.json` to get:
...
Reads `.codex-ray/intermediate/scan-result.json`, writes `.codex-ray/intermediate/batches.json`.
...
Load `.codex-ray/intermediate/batches.json` (produced by Phase 1.5). Iterate the `batches[]` array.
...
> Output: write to `$PROJECT_ROOT/.codex-ray/intermediate/batch-<batchIndex>.json` (single-file mode) OR `batch-<batchIndex>-part-<k>.json` (split mode, per Step B of your output protocol).
>
...
This script reads all `batch-*.json` files (including `batch-<i>-part-<k>.json` produced by file-analyzers that split their output) from `$PROJECT_ROOT/.codex-ray/intermediate/`, then in one pass:
...
Output: `$PROJECT_ROOT/.codex-ray/intermediate/assembled-graph.json`
...
```bash
git diff <lastCommitHash>..HEAD --name-only > $PROJECT_ROOT/.codex-ray/tmp/changed-files.txt
```

Run compute-batches with `--changed-files`:
```bash
node <SKILL_DIR>/compute-batches.mjs $PROJECT_ROOT \
  --changed-files=$PROJECT_ROOT/.codex-ray/tmp/changed-files.txt
```
...
> Review the assembled graph at `$PROJECT_ROOT/.codex-ray/intermediate/assembled-graph.json`.
> Project root: `$PROJECT_ROOT`
> Batch files are at: `$PROJECT_ROOT/.codex-ray/intermediate/batch-*.json`
> Write review output to: `$PROJECT_ROOT/.codex-ray/intermediate/assemble-review.json`
>
> **Merge script report:**
> ```
...
After the subagent completes, read `$PROJECT_ROOT/.codex-ray/intermediate/assemble-review.json` and add any notes to `$PHASE_WARNINGS`.
...
> Project root: `$PROJECT_ROOT`
> Write output to: `$PROJECT_ROOT/.codex-ray/intermediate/layers.json`
> Project: `<projectName>` — `<projectDescription>`
>
...
After the subagent completes, read `$PROJECT_ROOT/.codex-ray/intermediate/layers.json` and normalize it into a final `layers` array. Apply these steps **in order**:
...
> Project root: `$PROJECT_ROOT`
> Write output to: `$PROJECT_ROOT/.codex-ray/intermediate/tour.json`
> Project: `<projectName>` — `<projectDescription>`
> Languages: `<languages>`
>
...
After the subagent completes, read `$PROJECT_ROOT/.codex-ray/intermediate/tour.json` and normalize it into a final `tour` array. Apply these steps **in order**:
...
2. Write the assembled graph to `$PROJECT_ROOT/.codex-ray/intermediate/assembled-graph.json`.
...
Write the following Node.js script to `$PROJECT_ROOT/.codex-ray/tmp/ua-inline-validate.cjs`:
...
Execute it:
```bash
node $PROJECT_ROOT/.codex-ray/tmp/ua-inline-validate.cjs \
  "$PROJECT_ROOT/.codex-ray/intermediate/assembled-graph.json" \
  "$PROJECT_ROOT/.codex-ray/intermediate/review.json"
```
...
> Validate the knowledge graph at `$PROJECT_ROOT/.codex-ray/intermediate/assembled-graph.json`.
> Project root: `$PROJECT_ROOT`
> Read the file and validate it for completeness and correctness.
> Write output to: `$PROJECT_ROOT/.codex-ray/intermediate/review.json`

---

4. Read `$PROJECT_ROOT/.codex-ray/intermediate/review.json`.
...
1. Write the final knowledge graph to `$PROJECT_ROOT/.codex-ray/knowledge-graph.json`.
...
2. **Generate structural fingerprints baseline.** This creates the basis for future automatic incremental updates and **must succeed before `meta.json` is written** — otherwise auto-update sees a fresh commit hash with no fingerprints to compare against, classifies every file as STRUCTURAL, and escalates to `FULL_UPDATE` on every subsequent commit (issue #152).
   ...
   ```bash
   cat > $PROJECT_ROOT/.codex-ray/intermediate/fingerprint-input.json <<EOF
   {
   ...
   node <SKILL_DIR>/build-fingerprints.mjs \
     $PROJECT_ROOT/.codex-ray/intermediate/fingerprint-input.json
   ```
...
3. Write metadata to `$PROJECT_ROOT/.codex-ray/meta.json` (only after step 2 succeeded):
   ```json
   {
...
4. Clean up intermediate files:
   ```bash
   rm -rf $PROJECT_ROOT/.codex-ray/intermediate
   rm -rf $PROJECT_ROOT/.codex-ray/tmp
   ```
...
5. Report a summary to the user containing:
   ...
   - Path to the output file: `$PROJECT_ROOT/.codex-ray/knowledge-graph.json`
