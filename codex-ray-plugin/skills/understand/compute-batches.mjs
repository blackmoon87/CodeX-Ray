 ...
 *
 * Input:  <project-root>/.codex-ray/intermediate/scan-result.json
 * Output: <project-root>/.codex-ray/intermediate/batches.json
 */
...
let core;
try {
  core = await import(pathToFileURL(require.resolve('@codex-ray/core')).href);
} catch {
...
async function main() {
  ...
  const scanPath = join(projectRoot, '.codex-ray', 'intermediate', 'scan-result.json');
  ...
  const outPath = join(projectRoot, '.codex-ray', 'intermediate', 'batches.json');
