...
Start the CodeX-Ray dashboard to visualize the knowledge graph for the current project.
...
2. Check that `.codex-ray/knowledge-graph.json` exists in the project directory. If not, tell the user:
   ```
...
3. Find the dashboard code. The dashboard is at `packages/dashboard/` relative to this plugin's root directory. Check these paths in order and use the first that exists:
   ...
   - `~/.codex-ray-plugin/packages/dashboard/` (universal symlink, all installs)
   ...
   - Common clone-based install roots:
     - `~/.codex/CodeX-Ray/CodeX-Ray-plugin/packages/dashboard/`
     - `~/.opencode/CodeX-Ray/CodeX-Ray-plugin/packages/dashboard/`
     - `~/.pi/CodeX-Ray/CodeX-Ray-plugin/packages/dashboard/`
     - `~/CodeX-Ray/CodeX-Ray-plugin/packages/dashboard/`
...
4. Install dependencies and build if needed:
   ...
   ```bash
   cd <plugin-root> && pnpm --filter @codex-ray/core build
   ```
...
7. Report to the user, including the full tokenized URL:
   ...
   Viewing: <project-dir>/.codex-ray/knowledge-graph.json
