...
- **CodeX-Ray-plugin/** — Claude Code plugin containing all source code:
...
## Agent Pipeline
- Agents write intermediate results to `.codex-ray/intermediate/` on disk (not returned to context)
...
- `pnpm --filter @codex-ray/core build` — Build the core package
- `pnpm --filter @codex-ray/core test` — Run core tests
- `pnpm --filter @codex-ray/skill build` — Build the plugin package
- `pnpm test` — Run all tests (skill tests live at repo-root `tests/skill/`, picked up by root `vitest.config.ts`)
- `pnpm --filter @codex-ray/dashboard build` — Build the dashboard
...
- Knowledge graph JSON lives in `.codex-ray/` directory of analyzed projects
...
## Scripts
- `scripts/generate-large-graph.mjs` — Generates a fake knowledge graph for performance testing (e.g. large-graph layout). Writes to `.codex-ray/knowledge-graph.json`. Usage: `node scripts/generate-large-graph.mjs [nodeCount]` (default: 3000 nodes). Not part of the production pipeline.

## Versioning
When pushing to remote, bump the version in **all five** of these files (keep them in sync):
- `CodeX-Ray-plugin/package.json` → `"version"` field
- `CodeX-Ray-plugin/.claude-plugin/plugin.json` → `"version"` field
...
Claude Code caches installed plugins at `~/.claude/plugins/cache/CodeX-Ray/CodeX-Ray/<version>/`. Symlinks don't work because Claude's Search/Glob tools can't follow them. To test local changes:

1. **Build the packages:**
   ```bash
   pnpm --filter @codex-ray/core build
   pnpm --filter @codex-ray/skill build
   ```

2. **Find the installed version** (must match what the marketplace currently serves):
   ```bash
   ls ~/.claude/plugins/cache/CodeX-Ray/CodeX-Ray/
   ```

3. **Copy your local plugin into the cache**, replacing `<VERSION>` with the version from step 2:
   ```bash
   rm -rf ~/.claude/plugins/cache/CodeX-Ray/CodeX-Ray/<VERSION>
   cp -R ./CodeX-Ray-plugin ~/.claude/plugins/cache/CodeX-Ray/CodeX-Ray/<VERSION>
   ```
